...
def get_paths_dict(databases_loc, classes_df):
    ...
    for class_data in classes_df["Class"].to_list():
        class_unproc = f"{databases_loc}/{class_data}/unprocessed"
    ...
    VOGDB_unproc = f"{databases_loc}/VOGDB/unprocessed"
...
def ProcessHandler(arguments, classes_df):
    ...
    if arguments.all:
        ...
        for class_data, mmseqs_db_path in mmseqs_db_paths.items():
            ...
            process_db = f"{arguments.databases_loc}/{class_data}/processed"
        ...
        for class_data, m8_file in m8_files.items():
            ...
            meta = pd.read_csv(f"{arguments.databases_loc}/{class_data}/unprocessed/{class_data}.csv")
            edge_list = f"{arguments.databases_loc}/{class_data}/processed/edge_list.txt"
            ...
            out_hypergeom_edges = f"{arguments.databases_loc}/{class_data}/processed/hypergeom_edges.csv"
            if not os.path.exists(out_hypergeom_edges):
                ...
                pd.DataFrame({"source": sources, "target": destinations, "weight": weights}).to_csv(f"{arguments.databases_loc}/{class_data}/processed/hypergeom_edges.csv", index=False)
